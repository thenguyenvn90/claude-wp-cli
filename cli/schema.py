...
def register(subparsers):
    ...
    p.add_argument("--slug", default=None, help="expected slug; verified before the write (wrong-id guard)")
...
def handle(args, client: WPClient, config):
    ...
    if args.action == "get":
        ...
        if args.schema_file:
            schema_data = json_lib.loads(safe_read_text(args.schema_file))
        elif args.data:
